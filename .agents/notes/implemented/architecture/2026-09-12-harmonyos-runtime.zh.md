...
原生发行流程将 pnpm 和平台构建输入与运行依赖闭包分别固定。流程对上游鸿蒙扩展的私有副本签名，并连接到锁文件固定的对应使用方。Lightning CSS 缺少匹配的鸿蒙原生扩展，因此使用同版本上游 WASM API。复制安装避免签名和构建入口选择修改已准备的 pnpm 存储。Host、Client、Web 和系统扩展在鸿蒙上构建；上游构建工具二进制和 WASM 仍作为明确的外部编译输入。
