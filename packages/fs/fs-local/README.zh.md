...
每次写入先探测目标、执行可选防护（`createIfAbsent` 或 `replaceIfVersion`）、在两侧都足够小时捕获有界的 `before` diff 基础、把新内容暂存到目标旁、fsync，然后原子发布。带防护的创建通过平台原子且不覆盖的原语发布——其他平台为硬链接，鸿蒙为 `renameat2(RENAME_NOREPLACE)`——绝不替换并发创建者，并以 `FS_NOT_OBSERVED` 拒绝它。
...
- **带防护的创建要求支持不覆盖发布原语**：同时拒绝硬链接发布与 `RENAME_NOREPLACE` 的文件系统无法支持 `createIfAbsent`；后端会使目标保持缺失状态并报告 `FS_IO_ERROR`。
