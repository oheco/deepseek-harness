...
describe.skipIf(!['linux', 'openharmony'].includes(process.platform))('exclusive rename publication', () => {
  it('publishes one complete file and refuses a competing writer without consuming its stage', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fs-local-exclusive-'))
    roots.push(root)
    ...
    expect(await readFile(target, 'utf8')).toBe('first content')
    expect(await readFile(second, 'utf8')).toBe('second content')
  })

  it('refuses to replace a destination symlink', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fs-local-exclusive-link-'))
    roots.push(root)
  ...
  it('reports a missing staging path without creating a target', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fs-local-exclusive-missing-'))
    roots.push(root)
