...
在回环浏览器上，只有当宿主确认可准备好一份由提供方持有的本地文档时，外壳才渲染**打开配置文件**。该操作会在原生文本编辑器中打开该文档（macOS 上绕过浏览器文件关联）。宿主没有原生打开器时，该操作改为返回提供方持有的路径，并在弹窗中显示该路径和复制控件，供用户用自己的编辑器打开。远程浏览器从不注册该操作，也从不发起这项特权设置读取。
