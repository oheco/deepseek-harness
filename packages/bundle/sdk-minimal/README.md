...
On HarmonyOS, the persistent `bash` tool runs `/usr/bin/zsh` and tells the model to use zsh syntax. Python 3 with `termios` is required to disable terminal echo; the profile retains its explicit `danger-full-access` policy.
