...
- **独占发布依赖文件系统能力** — 鸿蒙应用目录禁止硬链接，因此使用 `renameat2(RENAME_NOREPLACE)`；其他 POSIX 宿主使用 `link()`，Windows 使用不替换目标的写穿透重命名。首次物化与代际发布均原子拒绝已有目标。鸿蒙在发布后同步父目录；若不支持所需重命名语义则失败，不会覆盖目标或复制不完整日志。
