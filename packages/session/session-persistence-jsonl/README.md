...
- **Exclusive publication depends on the filesystem** — HarmonyOS uses `renameat2(RENAME_NOREPLACE)` because application directories forbid hard links; other POSIX hosts use `link()`, and Windows uses write-through rename without replacement. First materialization and generation publication reject existing targets atomically. HarmonyOS syncs the parent directory after publication; unsupported rename semantics fail instead of overwriting or copying a partial log.
